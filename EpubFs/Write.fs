        ...
        [
            ...
            html [ xhtmlXmlns ] [
                head [] [
                    ...
                    for index, _ in List.indexed cssFiles do
                        link [ _href $"css{index + 1}"; _type "text/css"; _rel "stylesheet" ]
                ]
                body [] body'
            ]
        ]
...
let write (stream: Stream) metadata manifest =
    ...
    | NavInput.Stream s -> writeEntryStream archive CompressionLevel.Fastest "EPUB/_nav.xhtml" s
    | Autogenerated ->
    ...
    for x in manifest.TitlePage :: manifest.ContentFiles do
        ...
        match x.Input with
        | ContentInput.Stream s -> writeEntryStream archive CompressionLevel.Fastest name s
